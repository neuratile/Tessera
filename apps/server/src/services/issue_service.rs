...
pub async fn move_issue(
    ...
    // Re-read column/position under a row lock: a concurrent move could have
    // shifted this issue since the pre-transaction read, and the shift
    // predicates below would then operate on a stale range, duplicating or
    // gapping position values.
    let issue = sqlx::query_as::<_, IssueMoveInfo>(
        "SELECT board_id, column_id, position FROM issues WHERE id = $1 FOR UPDATE",
    )
    .bind(issue_id)
...
pub async fn delete_issue(
    ...
    // Authorization can use an unlocked read — the position arithmetic below
    // must not (see the locked re-read inside the transaction).
    let issue = sqlx::query_as::<_, IssueMoveInfo>(
        "SELECT board_id, column_id, position FROM issues WHERE id = $1"
    )
    .bind(issue_id)
    ...
    // Re-read column/position under a row lock: a concurrent move_issue could
    // have shifted this issue since the pre-transaction read, and shifting
    // neighbours by a stale position duplicates or gaps position values.
    let issue = sqlx::query_as::<_, IssueMoveInfo>(
        "SELECT board_id, column_id, position FROM issues WHERE id = $1 FOR UPDATE",
    )
    .bind(issue_id)
